...
<template>
  ...
  >
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div class="space-y-6">
        ...
        <div class="p-4 bg-gray-50/50 rounded-xl border border-gray-100">
          <label
            class="flex items-center text-xs font-black uppercase tracking-wider text-gray-400 mb-3 ml-1"
          >
            <svg
        ...
        <div class="p-4 bg-gray-50/50 rounded-xl border border-gray-100">
          ...
          <select v-model="form.target_id" class="input-modern-sm">
            <option
              v-for="pt in PREDEFINED_TARGETS"
              :key="pt.id"
              :value="pt.id"
            >
              {{ pt.name }}
      ...
      <div class="space-y-6">
        <!-- 3. GIÁ TRỊ -->
        <div
          class="p-4 bg-blue-50/30 rounded-xl border border-blue-100 h-full flex flex-col justify-center"
        >
          <label
            class="flex items-center text-xs font-black uppercase tracking-wider text-blue-500 mb-4 ml-1"
          >
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
          ...
          <div class="relative group">
            <input
              type="text"
              :value="
                form.amount === 0 ? '' : form.amount.toLocaleString('vi-VN')
              "
            ...
            />
            <div
              class="absolute right-6 top-1/2 -translate-y-1/2 text-xs font-black text-blue-400 uppercase tracking-widest bg-blue-50 px-2.5 py-1 rounded-lg"
            >
          ...
          <div class="mt-4 flex items-center space-x-2 px-1">
            ...
            <span class="text-[10px] font-black text-indigo-600 uppercase">{{
              getTargetInfo(form.target_id)?.type
            }}</span>
          </div>
        </div>
      </div>
    ...
    <div
      class="mt-8 pt-6 border-t border-gray-50 flex justify-between items-center"
    >
