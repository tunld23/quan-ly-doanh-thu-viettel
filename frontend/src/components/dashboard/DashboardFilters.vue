<template>
  <div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
      <div class="flex flex-col md:flex-row md:items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/20 gap-6">
        ...
        <div class="flex items-center gap-10 min-w-0 flex-1">
          <div class="flex flex-col items-start min-w-0 w-full">
            <span class="text-[10px] font-bold text-blue-500/60 uppercase tracking-[0.2em] mb-1 px-0.5">Danh mục</span>
            <div class="flex items-center gap-8 overflow-x-auto category-scrollbar pb-2 -mb-2 w-full">
              <button
              ...
              >
                <span
                  ...
                  :class="dataType === mode.id ? 'text-gray-900' : 'text-gray-400 group-hover:text-gray-600'"
                >
                  {{ mode.name }}
                </span>
                <div
                  class="absolute bottom-0 left-0 right-0 h-[3px] bg-blue-600 rounded-full transition-all duration-300 transform origin-left"
                  :class="dataType === mode.id ? 'scale-x-100 opacity-100' : 'scale-x-0 opacity-0 group-hover:scale-x-50 group-hover:opacity-30'"
                ></div>
              </button>
            </div>
          </div>
        ...
        <div class="flex flex-wrap items-center gap-6 py-4 md:py-0">
          ...
          <!-- Year Selector -->
          <div class="flex flex-col items-start">
            <span class="text-[11px] font-bold text-gray-400 uppercase tracking-[0.12em] mb-2.5 px-0.5">Thời gian</span>
            <div class="flex items-center gap-3 bg-white p-1 rounded-2xl border border-gray-200/60 shadow-sm">
              <select
                :value="selectedYear"
              ...
              <div class="flex items-center p-0.5 bg-gray-50 rounded-xl">
                <button
                  ...
                  :class="filterMode === mode.id ? 'bg-white shadow-sm text-blue-600 ring-1 ring-black/5' : 'text-gray-500 hover:text-gray-700'"
                >
                  {{ mode.name }}
                </button>
              </div>
          ...
          <div v-if="filterMode === 'month'" class="flex items-center gap-2 animate-in slide-in-from-left-2 duration-300">
            <span class="text-gray-500 font-medium ml-2 border-l border-gray-200 pl-4">Chọn tháng:</span>
            <select
              :value="selectedMonth"
          ...
          <div v-if="filterMode === 'quarter'" class="flex items-center gap-3 animate-in slide-in-from-left-2 duration-300">
            <span class="text-gray-500 font-medium ml-2 border-l border-gray-200 pl-4">Chọn quý:</span>
            <div class="flex items-center border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm">
              <button
                ...
                :class="selectedQuarter === q ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'"
              >
                Q{{ q }}
              </button>
              <button
                ...
                :class="selectedQuarter === '' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-50'"
              >
                Tất cả
              </button>
            </div>
...
const sourceOptions = [
  { id: 'all', name: 'Tất cả', icon: 'M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25a2.25 2.25 0 01-2.25 2.25h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25h-2.25a2.25 2.25 0 01-2.25-2.25v-2.25z' },
  { id: 'dealer', name: 'Đại lý', icon: 'M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z' },
  { id: 'am', name: 'Account Manager (AM)', icon: 'M15 9a3 3 0 11-6 0 3 3 0 016 0zM6.343 17.403a.75.75 0 01.196-.306 7.5 7.5 0 119.924 0 .75.75 0 01.196.306l1.274 4.053a.75.75 0 01-.969.94l-3.974-1.49a.75.75 0 00-.518 0l-3.974 1.49a.75.75 0 01-.969-.94l1.274-4.053z' }
];
...
const processedDataModes = computed(() => {
  const groups = props.productGroups?.length > 0 ? props.productGroups : ["all"];
  return groups.map((g) => ({
    id: g,
