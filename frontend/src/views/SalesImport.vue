...
<template>
  <div class="sales-import-container p-6">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="bg-white rounded-xl shadow-lg p-8 border border-gray-100 backdrop-blur-sm bg-white/90">
        <!-- 1. Source Selection -->
        <div class="mb-8 p-4 bg-gray-50/50 rounded-xl border border-gray-200">
          ...
          <div class="grid grid-cols-2 gap-4">
            <button @click="selectedSource = 'dealer'" :class="selectedSource === 'dealer' ? 'active-btn' : 'inactive-btn'">
              <span class="text-xs uppercase tracking-widest">Đại Lý</span>
              <span class="text-[10px] font-normal opacity-70 italic">Ủy Quyền</span>
            </button>
            <button @click="selectedSource = 'am'" :class="selectedSource === 'am' ? 'active-btn' : 'inactive-btn'">
        ...
        <div class="mb-8 p-4 bg-gray-50/50 rounded-xl border border-gray-200">
          <label class="flex items-center text-xs font-black uppercase tracking-wider text-gray-500 mb-4 ml-1">
            <svg class="w-5 h-5 mr-2 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
          ...
          <!-- Rules Display -->
          <div v-if="selectedType" class="mt-4 p-5 bg-blue-50/50 rounded-2xl border border-blue-100">
             <div class="flex items-start space-x-3 text-xs text-blue-800 leading-relaxed">
               ...
               <div>
                 <p class="font-black uppercase tracking-widest mb-2 italic">Quy ước nạp: <span class="text-blue-900 not-italic ml-1">{{selectedType}}</span></p>
                 <ul class="space-y-1.5 ml-1">
                   <li v-for="rule in (rules[selectedType] || defaultRule)" :key="rule.label" class="flex items-center gap-1.5">
                     <div class="w-1 h-1 bg-blue-400 rounded-full"></div> <b>{{rule.label}}:</b> {{rule.rule}}
                   </li>
                 </ul>
               </div>
             </div>
        ...
        <div v-if="requiresManualDate" class="mb-8 p-6 bg-indigo-50/50 rounded-2xl border border-indigo-100 flex items-center gap-6 animate-in slide-in-from-top-2 duration-300">
          <div class="flex-shrink-0 bg-indigo-600 p-3 rounded-xl shadow-lg shadow-indigo-200">
          ...
          <div class="flex-1 grid grid-cols-2 gap-6">
            <div class="space-y-1.5">
              <label class="block text-[11px] font-black text-indigo-900/60 uppercase tracking-[0.1em] ml-1">Tháng</label>
              <select v-model="manualMonth" class="date-select">
                <option v-for="m in 12" :key="m" :value="String(m).padStart(2, '0')">Tháng {{ m }}</option>
              </select>
            </div>
            <div class="space-y-1.5">
              <label class="block text-[11px] font-black text-indigo-900/60 uppercase tracking-[0.1em] ml-1">Năm</label>
...
<style scoped>
.active-btn { @apply py-4 rounded-xl border-2 border-indigo-600 bg-indigo-50 text-indigo-700 shadow-md scale-[1.02] flex flex-col items-center justify-center space-y-1 font-bold transition-all; }
.inactive-btn { @apply py-4 rounded-xl border-2 border-gray-100 bg-gray-50 text-gray-500 hover:border-indigo-200 flex flex-col items-center justify-center space-y-1 font-bold transition-all; }
.input-modern {
